...
var self = module.exports = {
	...
	helpListParser: function(list, categories) {
		...
		fields.forEach((fieldItem, fieldIndex, fieldArray) => {
	...
	helpGetParser: function(documentation) {
		let fields = [];
		fields.push({ name: 'Category', value: `${documentation.category}`, inline: true });
		if (documentation.description) {
			fields.push({ name: 'Description', value: documentation.description, inline: true });
		} else {
			fields.push({ name: 'Description', value: '*No description provided*', inline: true });
		};
		if (documentation.syntax) {
			fields.push({ name: 'Syntax', value: documentation.syntax });
		} else {
			fields.push({ name: 'Syntax', value: '*No syntax provided*' });
		};
		if (documentation.cooldown) {
			fields.push({ name: 'Cooldown', value: documentation.cooldown });
		} else {
			fields.push({ name: 'Cooldown', value: '*No cooldown*' });
		};
		if (documentation.arguments.length != 0) {
			let args = '';
			documentation.arguments.forEach((item, index, array) => {
			...
			fields.push({ name: 'Arguments', value: args });
		} else {
			fields.push({ name: 'Arguments', value: '*No arguments*' });
		};
		return fields;
	}
};