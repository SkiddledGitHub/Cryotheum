...
var self = module.exports = {
    getConfigPath: function() {
        if (fs.existsSync(`${process.cwd()}/setup.js`)) {
            return process.cwd();
        } else if (fs.existsSync('../setup.js')) {
                return '../';
        }
    },
    validate: function() {
        ...
        let loggingMsgStatus = ((config.loggingMessages || config.loggingMessages === false) || config.loggingMessages != undefined) ? true : false
        ...
        if (!config.botAuth || !config.botID || !config.botOwner || !loggingMsgStatus || !debugStatus || !githubAuthStatus || !specialBadgesStatus) {
