...
const stdCooldown = new Set();
const stdCooldownTime = 8000;
...
module.exports = {
  ...
  async execute(interaction) {
      try {
      if (stdCooldown.has(interaction.user.id)) {
      await interaction.reply({ 
        ...
        await interaction.reply({ 
          ...
          console.log(`\x1b[1;32m==> \x1b[1;37m${member} sent a message to stdout: \n\x1b[0m\x1b[35m -> \x1b[37m${message}`);
          }
        };
      // add user to cooldown
      stdCooldown.add(interaction.user.id);
        setTimeout(() => {
          // rm cooldown after it has passed
          stdCooldown.delete(interaction.user.id);
        }, stdCooldownTime);
      }
    } catch (error) {
      await interaction.reply({
